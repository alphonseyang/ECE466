...
public class Buffer 
{
	...
	private BlockingQueue<Long> departureTimes;
	...
	public Buffer(long bufferCapacity)
	{
		...
		departureTimes = new LinkedBlockingQueue<Long>();
		sizeInBytes = 0L;
	 ...
	 * @param delay Departure delay of this packet.
	 * @return Size of buffer in bytes <b>after</b> adding the packet (or -1).
	 */
	public synchronized long addPacket(DatagramPacket packet, double delay)
	{
		if (increaseSize(packet.getLength()))
		{
			queue.add(packet);
			departureTimes.add((long) (System.nanoTime()+delay*1000000));
	...
	public synchronized long getWaitTimeUntilDeparture()
	{
		return departureTimes.peek() - System.nanoTime();
	}
